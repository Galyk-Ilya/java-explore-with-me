...
import ru.practicum.event.dto.StateActionForAdmin;
import ru.practicum.event.dto.StateActionForUser;
...
public class EventServiceImpl implements EventService {
    ...
    public EventFullDto addEvent(NewEventDto newEventDto, Long id) {
        if (newEventDto.getEventDate() != null && !LocalDateTime
            ...
            throw new NotValidException("The start of the event is not suitable.");
        }

        if (LocalDateTime.parse(newEventDto.getEventDate(), ofPattern(Patterns.DATE_PATTERN))
                .isBefore(LocalDateTime.now().plusHours(2))) {
            throw new WrongEventDateException("The start of the event cannot be earlier than in " +
                    "two hours from now.");
        }
        ...
        User user = userRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Invalid user id data"));
    ...
    public EventFullDto updateEventForAdmin(Long eventId, UpdateEventAdminRequestDto updateEventAdminRequestDto) {
        ...
        if (updateEventAdminRequestDto.getEventDate() != null && LocalDateTime.parse(updateEventAdminRequestDto.getEventDate(), ofPattern(Patterns.DATE_PATTERN))
                .isBefore(LocalDateTime.now().plusHours(2))) {
            throw new NotValidException("The event date must be at least two hours later than the current event.");
        }

        Event event = eventRepository.findById(eventId)
                .orElseThrow(() -> new NotFoundException("No such event exists"));
        ...
        if (event.getState().equals(EventState.PUBLISHED) &&
            ...
            throw new DataIntegrityViolationException("Event already published");
                ...
                updateEventAdminRequestDto.getStateAction().equals(StateActionForAdmin.PUBLISH_EVENT)) {
            throw new DataIntegrityViolationException("Cannot publish a canceled event");
            ...
            throw new DataIntegrityViolationException("Cannot cancel a published event. Show must go on!");
        }
    ...
    public List<EventShortDto> findEventsForPublic(String text, List<Long> categories, Boolean paid,
        ...
        if (rangeStart != null && rangeEnd != null) {
            if (LocalDateTime.parse(rangeStart, ofPattern(Patterns.DATE_PATTERN))
                ...
                throw new NotValidException("The event time interval is not set correctly");
            }
        }
